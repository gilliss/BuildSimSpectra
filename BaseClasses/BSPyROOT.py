...
class BSPyROOT():
    def __init__(self, BSCurrentVarsObject):
        ...
        self.xmax = 10000.0 # upper edge (excluded in ROOT, included in np and plt)
    ...
    def GetBinnedData(self, inFile, **currentVarsDict):
        ...
        if (h.GetNbinsX() != self.nBinsX) or (h.GetNcells() != self.nBinsX + 2):
            self.Print(0, 'Error', '  GetBinnedData: Unexpected binning', inFile, hName)

        hArray = np.zeros(self.nBinsX)
        if h.GetEntries() > 0:
            self.Print(3, 'Debug', '  Working with:', h.GetName(), h.GetTitle(), h.GetNbinsX(), h.GetEntries(), h.Integral())
            hArray = np.frombuffer(h.GetArray(), dtype = 'float', count = self.nBinsX + 2, offset = 0) # getting array of data from PyDoubleBuffer object
...
# NOTES
    ...
    # All but the last (righthand-most) bin is half-open. The first bin is [1, 2) -- [including, excluding)-- and the second [2, 3). The last bin, however, is [3, 4], which includes 4.
    # where : [ ‘pre’ | ‘post’ | ‘mid’ ]
    ...
    # fNcells; ///< number of bins(1D), cells (2D) +U/Overflows
    #
